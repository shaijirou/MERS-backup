...
// Fetch firefighter personnel profile picture
$firefighter_id = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null;
...
try {
    if ($firefighter_id) {
        ...
        $firefighter_data = $stmt->fetch(PDO::FETCH_ASSOC);

        if ($firefighter_data && !empty($firefighter_data['id_document'])) {
            // Use absolute path for file_exists, but relative path for src attribute
            $profile_picture_filename = $firefighter_data['id_document'];
...
<nav class="navbar navbar-expand-lg navbar-light bg-light border-bottom">
    <div class="container-fluid">
        ...
        <h4 style="margin-left: 8px;"><?php echo isset($_SESSION['user_name']) ? htmlspecialchars($_SESSION['user_name']) : 'Fire Personnel'; ?></h4>
